...
TOOLS = [
    {
        ...
        "function": {
            "name": "web_search",
            "description": (
                "Ищет информацию в интернете. Инструмент дорогой и медленный — "
                "вызывай его только когда ответ ЗАВИСИТ ОТ ТОГО, ЧТО ПРОИСХОДИТ СЕЙЧАС.\n"
                "ИСПОЛЬЗУЙ: новости и события; текущие цены, курсы, статистика; последние "
                "версии, релизы и даты выхода; расписания; вопросы со словами «сейчас», "
                "«сегодня», «последний», «новый», «актуальный»; а также когда пользователь "
                "прямо просит найти, проверить или посмотреть в интернете.\n"
                "НЕ ИСПОЛЬЗУЙ: объяснение понятий и теории; написание, разбор и отладка "
                "кода; поиск причины ошибки; математика; переводы; определения; общие "
                "знания о языках программирования, алгоритмах и технологиях; болтовня и "
                "приветствия. На такое отвечай сам — ты это знаешь.\n"
                "Незнакомое или бессмысленное слово искать НЕ НАДО: честно скажи, что "
                "такого термина не знаешь, и попроси уточнить, что имелось в виду.\n"
                "Если поиск всё же нужен — передавай сразу НЕСКОЛЬКО разных формулировок "
                "одного вопроса, они ищутся параллельно. Можно вызвать повторно, если "
                "после первых результатов остались пробелы."
            ),
            "parameters": {
                "type": "object",
                "properties": {
